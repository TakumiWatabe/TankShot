...
void Game::Initialize(HWND window, int width, int height)
{
	...
	*/
	...
	Enemy* pEnemies[ENEMY_NUM];

	//for (int i = 0; i < ENEMY_NUM; i++)
	//{
	//	pEnemies[i] = new Enemy;
	//	pEnemies[i]->Initialize();
	//	pEnemies[i]->Update();
	//	pEnemies[i]->Draw();
	//}
	//==========Initialization=========
	...
	//天球モデルの読み込み
	m_objSkydorm.LoadModel(L"Resources/skydorm.cmo");
	//地面モデルの読み込み
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	{//自機が地面に立つ処理
		if (m_player->GetVeclocity().y <= 0.0f)
		{
			//自機の頭から足元への線分
	...
	m_player->SetKeyBord(keyboard->GetState());
	m_player->Update();
	...
	//弾丸と敵の当たり判定
	{
		...
		for (std::vector<std::unique_ptr<Enemy>>::iterator it = m_enemies.begin(); it != m_enemies.end();)
		{
			...
			{
				ModelEffectManager::getInstance()->Entry(L"Resources/effect.cmo", 10, Vector3(enemy->GetParentTrans()), Vector3(0, 0, 0), Vector3(0, 0, 0), Vector3(0, 0, 0), Vector3(0, 0, 0), Vector3(0, 0, 0), Vector3(3, 3, 3));
...
void Game::Render()
{
	...
	if (m_enemies.size() == 0)
	{
		spriteFont->DrawString(spriteBatch.get(), L"Clear!", XMFLOAT2(0, 120));
	}
	spriteFont->DrawString(spriteBatch.get(), L"A : LeftTurn", XMFLOAT2(0, 0));
	spriteFont->DrawString(spriteBatch.get(), L"D : RightTurn", XMFLOAT2(0, 20));
	spriteFont->DrawString(spriteBatch.get(), L"W : Front", XMFLOAT2(0, 40));
	spriteFont->DrawString(spriteBatch.get(), L"S : Back", XMFLOAT2(0, 60));

	spriteFont->DrawString(spriteBatch.get(), L"Space : ShootBullet", XMFLOAT2(0, 80));
